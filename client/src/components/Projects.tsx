...
import kapture1 from "../assets/kapture-1.png";
import kapture2 from "../assets/kapture-2.png";
import kapture3 from "../assets/kapture-3.png";
import kapture4 from "../assets/kapture-4.png";
import kapture5 from "../assets/kapture-5.png";
import kapture6 from "../assets/kapture-6.png";
import kapture7 from "../assets/kapture-7.png";
...
import senseminder1 from "../assets/senseminder-1.png";
import senseminder2 from "../assets/senseminder-2.png";
import senseminder3 from "../assets/senseminder-3.png";
import senseminder4 from "../assets/senseminder-4.png";
import senseminder5 from "../assets/senseminder-5.png";

// Animated Kapture Project Component
const KaptureProject = ({ project }: { project: any }) => {
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  
  const kaptureImages = [
    kapture1, kapture2, kapture3, kapture4, kapture5, kapture6, kapture7
  ];

  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentImageIndex((prev) => (prev + 1) % kaptureImages.length);
    }, 2000);
...
// Animated SenseMinder Project Component
const SenseMinderProject = ({ project }: { project: any }) => {
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  
  const senseminderImages = [
    senseminder1, senseminder2, senseminder3, senseminder4, senseminder5
  ];

  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentImageIndex((prev) => (prev + 1) % senseminderImages.length);
    }, 2000);
    return () => clearInterval(interval);
  }, []);
  ...
  return (
    <div className="group bg-dark-secondary rounded-3xl overflow-hidden hover:shadow-2xl hover:shadow-emerald-500/20 transition-all duration-700 hover:scale-[1.02] hover:-translate-y-2 relative border border-emerald-500/30 hover:border-emerald-500">
      <div className="relative h-72 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-t from-dark-secondary via-transparent to-transparent z-10"></div>
        <div className="relative w-full h-full">
          {senseminderImages.map((image, index) => (
            <img
              key={index}
              src={image}
              alt={`${project.title} Screenshot ${index + 1}`}
